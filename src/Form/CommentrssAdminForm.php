  ...
  public function buildForm(array $form, array &$form_state) {
    ...
    $form['site'] = array(
      ...
      '#options' => array(
        ...
        COMMENTRSS_SITE_FRONT_AND_NODE_PAGE => t('Enabled, expose on front page and <a href="@url">default content listing</a> page', array('@url' => $this->url('node'))),
        COMMENTRSS_SITE_ALL_PAGES => $this->t('Enabled, expose on all pages'),
      ),
      '#description' => $this->t('Where should the site comment feed be exposed, if at all enabled. If enabled, feed will be accessible at @url.', array('@url' => $this->url('crss', array('absolute' => TRUE)))),
      '#default_value' => $config->get('site'),
    );
    $form['node'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Enable individual comment feeds for each post on the website'),
      '#description' => $this->t('Feeds will be accessible on @url type of URLs.', array('@url' => $this->url('crss/node/...', array('absolute' => TRUE)))),
      '#default_value' => $config->get('node'),
    );
    $form['term'] = array(
      '#type' => 'checkbox',
      '#title' => $this->t('Enable individual comment feeds for each taxonomy term listing on the website'),
      '#description' => $this->t('Feeds will be accessible at @url type of URLs. Only supports one term, no composition.', array('@url' => $this->url('crss/term/...', array('absolute' => TRUE)))),
